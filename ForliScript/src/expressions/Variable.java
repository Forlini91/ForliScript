 ...
 * @author MarcoForlini
 */
public class Variable implements VariableValue, VariableContent, SubValue  {
	 ...
	 * @param constant	If true, the variable is constant
	 */
	public Variable (String name, Primitive value, boolean constant) {
		this.name = name;
		this.value = value;
	...
	public Variable (String name, Value value, boolean constant){
		this(name, value.getPrimitive(), constant);
	}
	...
	public Primitive set (Value newValue) throws ExecutionException {
		...
		return value = newValue.getPrimitive();
	}
	
	
	@Override
	public Primitive getNotNull() {
		if (value == Null){
	...
	public Value get (int i) throws ExecutionException {
		Primitive prim = getNotNull();
		if (prim instanceof SubValueRead) {
			return ((SubValueRead) prim).get(i);
		}
		throw accessSubValueException.apply(prim);
	}
	...
	public Value set (int i, Value newValue) throws ExecutionException {
		...
		Primitive prim = getNotNull();
		if (prim instanceof SubValue){
			return ((SubValue) prim).set(i, newValue);
		}
		throw accessSubValueException.apply(prim);
	}
